...
class wtc_trial_balance_report_print(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(wtc_trial_balance_report_print, self).__init__(
...
#                         p.update({'lines': account_move_lines})
        ...
        super(wtc_trial_balance_report_print, self).set_context(
    ...
    def formatLang_zero2blank(self, value, digits=None, date=False, date_time=False,
        ...
        if isinstance(value, (float, int)) and not value:
            ...
            return super(wtc_trial_balance_report_print, self).formatLang(
                value, digits, date, date_time, grouping,
                monetary, dp, currency_obj)

class wrapped_vat_declaration_print(orm.AbstractModel):
    _name = 'report.wtc_account_move.report_trial_balance'
    _inherit = 'report.abstract_report'
    _template = 'wtc_account_move.report_trial_balance'
    _wrapped_report_class = wtc_trial_balance_report_print
