...
class B2bDgiBranchConfig(models.Model):
    ...
    def _get_data_order(self):
        ...
        if self.config_id.code == "WMS H1":
            vals = {
                'no_prospect':self.no_prospect,
                'no_spk':self.no_spk,
                'date':self.date,
            }
            return self.config_id.suspend_security().action_manual_data_order_wahana_h1(self.branch_id, vals)             
        return order
...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def action_manual_data_order_wahana_h1(self, branch, vals):
        query_date = datetime.strptime(vals.get('date'),'%Y-%m-%d')
        spks_result = self._get_data_oder_wahana_h1(branch, query_date, log=False, idProspect=vals.get('no_prospect'), idSpk=vals.get('no_spk'))
        ...
        proses = self._process_data_order_wahana_h1(branch, datas, log=False)
        return proses