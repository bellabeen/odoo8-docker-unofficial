...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def _get_api_lsng_wahana(self, branch, query_date, log=True, idProspect=False, idSpk=False):
        api_url = "%s/lsng/read" % self.base_url
        date_format = '%Y-%m-%d %H:%M:%S'

        from_time = datetime.combine(
            query_date - timedelta(days=7), datetime.min.time())
        ...
        if not self.api_key and self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H1 SPK Wahana',api_url,'post',error,'LSNG')
        ...
        response = self.post(name="DGI H1 LSNG Wahana",url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
