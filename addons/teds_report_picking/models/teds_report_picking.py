...
class wtc_report_stock_picking_wizard(osv.osv_memory):
   
    _name = "wtc.report.stock.picking.wizard"
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(wtc_report_stock_picking_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'options': fields.selection([('outstanding','Outstanding')], 'Options', required=True, change_default=True, select=True),
        ...
        'branch_ids': fields.many2many('wtc.branch', 'wtc_report_stock_picking_branch_rel', 'wtc_report_stock_picking_wizard_id',
            'branch_id', 'Branches', copy=False),
        'categ_ids': fields.many2many('product.category', 'wtc_report_stock_picking_categ_rel', 'wtc_report_stock_picking_wizard_id',
            'categ_id', 'Categories', copy=False, domain=[('type','=','normal')]),
        'product_ids': fields.many2many('product.product', 'wtc_report_stock_picking_product_rel', 'wtc_report_stock_picking_wizard_id',
            'product_id', 'Products'),
        'partner_ids': fields.many2many('res.partner', 'wtc_report_stock_picking_partner_rel', 'wtc_report_stock_picking_wizard_id',
            'partner_id', 'Partners'),     
    }
    ...
    def _print_excel_report(self, cr, uid, ids, data, context=None):        
        ...
        if date_start_date :
            query_where += "  AND date(spick.create_date) >= '%s'" % str(date_start_date)
        if date_end_date :
            query_where += "  AND date(spick.create_date) <= '%s'" % str(date_end_date)
        if branch_ids :
        ...
        filename = 'Report Stock Picking '+str(date)+'.xlsx'        
        worksheet.write('A1', company_name , wbf['company'])
        worksheet.write('A2', 'Report Stock Picking' , wbf['title_doc'])
        worksheet.write('A3', 'Tanggal : %s s/d %s'%(str(date_start_date),str(date_end_date)) , wbf['company'])
        row=3
        rowsaldo = row
        row+=1
        ...
        worksheet.write('U%s' % (row+1), 'Picking State' , wbf['header'])
        ...
        worksheet.merge_range('S%s:X%s' % (row,row), '', wbf['total']) 
        ...
        form_res = ir_model_data.get_object_reference(cr, uid, 'teds_report_picking', 'view_report_stock_picking_wizard')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'wtc.report.stock.picking.wizard',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
...
wtc_report_stock_picking_wizard()
