...
class reportCdb(osv.osv_memory):
    _name = "export.cdb.txt"
    _description = "Export CDB TXT"
    ...
    _columns = {
        ...
        'options':fields.selection([('cdb', 'CDB')]),
        'branch_ids': fields.many2many('wtc.branch', 'export_cdb_txt_rel', 'export_cdb_txt_wizard_id',
                                        'branch_id', 'Branch', copy=False),
    }

    _defaults = {
        'state_x': lambda *a: 'choose',
        'options': lambda *a: 'cdb',
    ...
    def csv_report(self, cr, uid, ids, context=None):
        ...
        if data['options'] == 'ustk' :
            self._print_ustk(cr, uid, ids, data, context=context)
        else :
            self._print_cdb(cr, uid,ids,data,context=context)

        
        ir_model_data = self.pool.get('ir.model.data')
        form_res = ir_model_data.get_object_reference(cr, uid, 'teds_export_cdb_txt', 'export_cdb_txt_form_view')

        form_id = form_res and form_res[1] or False
        return {
            'name': _('Export CDB to txt'),
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'export.cdb.txt',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
    ...
    def _print_ustk(self, cr, uid, ids, data, context=None):
        return True
        # data = self.read(cr, uid, ids,context=context)[0]
    ...
    def _print_cdb(self, cr, uid, ids, data, context=None):
        data = self.read(cr, uid, ids,context=context)[0]
        this = self.browse(cr, uid, ids)[0]
        ...
        #     file_value = file_data.getvalue()
        #     out=base64.encodestring(file_value)
        #     filename = 'Export %s %s-%s' %(data['options'],data['start_date'],data['end_date'])
        #     self.write(cr, uid, ids,
        #                {'state_x':'get', 'data_x':out, 'name': filename +".txt"},
        #                context=context)
        # finally:
