...
from .wtc_report_faktur_pajak_others import wtc_report_faktur_pajak_others_print
from openerp.tools.translate import translate
import string

class wtc_report_faktur_pajak_others_print_xls(wtc_report_faktur_pajak_others_print):

    def __init__(self, cr, uid, name, context):
        super(wtc_report_faktur_pajak_others_print_xls, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'datetime': datetime,
            'wanted_list_overview': self.pool.get('wtc.faktur.pajak.other')._report_xls_faktur_pajak_others_fields(cr, uid, context),
            '_': self._,
        })
...
class report_faktur_pajak_others_xls(report_xls):

    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(report_faktur_pajak_others_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_overview = {
            ...
            'no_faktur': {
                'header': [1, 22, 'text', _render("_('No Faktur Pajak')")],
                'lines': [1, 0, 'text', _render("p['no_faktur']")],
                'totals': [1, 22, 'number', None]},
            'partner_code': {
                'header': [1, 15, 'text', _render("_('Partner Code')")],
            ...
            'date': {
                ...
                'totals': [1, 22, 'number', None]},
            'pajak_gabungan': {
                'header': [1, 10, 'text', _render("_('Pajak Gabungan')")],
                'lines': [1, 0, 'text', _render("p['pajak_gabungan']")],
                'totals': [1, 22, 'number', None]},
            'thn_penggunaan': {
                'header': [1, 10, 'text', _render("_('Tahun Penggunaan')")],
                'lines': [1, 0, 'text', _render("p['thn_penggunaan']")],
                'totals': [1, 22, 'number', None]},
            'tgl_terbit': {
                ...
                'totals': [1, 22, 'number', None]},
            'no_kwitansi': {
                'header': [1, 18, 'text', _render("_('No Kwitansi')")],
                'lines': [1, 0, 'text', _render("p['no_kwitansi']")],
                'totals': [1, 22, 'number', None]},                                        
            'untaxed_amount': {
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for r in _p.reports:
            ws_o = wb.add_sheet('Laporan Faktur Pajak Others')
            ...
            c_specs_o = [('report_name', 1, 0, 'text', 'Laporan Faktur Pajak Others')]
            row_data = self.xls_row_template(c_specs_o, ['report_name'])
            row_pos_o = self.xls_write_row(ws_o, row_pos_o, row_data, row_style=cell_style)
...
report_faktur_pajak_others_xls('report.Laporan Faktur Pajak Others', 'wtc.faktur.pajak.other', parser = wtc_report_faktur_pajak_others_print_xls)
