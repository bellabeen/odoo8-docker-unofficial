...
class B2bDgiBranchConfig(models.Model):
    ...
    def _get_data_work_order(self):
        ...
        if self.config_id.code == "MPM":
            vals = {
                'no_work_order':self.no_wo,
                'date':self.date,
                'type':self.type_h23,
            }
            return self.config_id.suspend_security().action_manual_data_work_order_mpm_h23(self.branch_id, vals)             
        return order
...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def action_manual_data_work_order_mpm_h23(self, branch, vals):
        query_date = datetime.strptime(vals.get('date'),'%Y-%m-%d')
        if vals.get('type') == 'PRSL':
            pkb_result = self._get_data_prsl_mpm_h23(branch, query_date, log=False, noWorkOrder=vals.get('no_work_order'))
            ...
            proses = self._process_data_prsl_mpm_h23(branch, datas, log=False)
            return proses
        else:
            pkb_result = self._get_data_wo_mpm_h23(branch, query_date, log=False, noWorkOrder=vals.get('no_work_order'))
            datas = pkb_result.get('data')
            if pkb_result.get('status', 0)==0 or not datas:
            ...
            # proses data create sale order draft        
            proses = self._process_data_wo_mpm_h23(branch, datas, log=False)
            return proses