...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def _get_api_pkb_mpm(self, branch, query_date, log=True, noWorkOrder=False):
        ...
        if not self.api_key and not self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H23 PKB MPM',api_url,'post',error,'PKB')
        ...
        response = self.post(name="DGI H23 PKB MPM", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
        if response.status_code == 200:
            ...
            data = [d for d in data if d.get('statusWorkOrder') in ('FINISH') and d.get('noWorkOrder') not in registered_wo]
            ...
            if log:
                self.create_log_error_dgi('DGI H23 WO MPM',api_url,'post',error,'PKB')
            return {'status':0,'error':error}

    @api.multi
    def _get_data_wo_mpm_h23(self, branch, query_date=False, log=True, noWorkOrder=False):
        try:
            #default date is today (for scheduler)
            if not query_date:
                query_date = datetime.now()
            #get wo yang status completed/Approved dan belum di proses sebelumnya
            wos = self._get_api_pkb_mpm(branch, query_date, log, noWorkOrder)
                ...
                for wo in wos:
                    ...
                    if noWorkOrder not in daftar_wo:
                        ...
                        if len(services) == 0 and len(parts) == 0:
                            ...
                            if not log:
                                raise Warning(error)
                            self.create_log_error_dgi('DGI Data Work Order MPM',self.base_url,'post',error,'PKB')
                            continue
            ...
            _logger.warning("Exception DGI Data Work Order MPM >>>>>>>>> %s"%(err))
            if not log:
                raise Warning(err)
            self.create_log_error_dgi('Exception DGI Data Work Order MPM',self.base_url,'post',err,'PKB')

    @api.multi
    def _process_data_wo_mpm_h23(self, branch, datas, log=True):
        ...
        for data in datas:                
            ...
            if not lot_obj:
                ...
                if not product_unit_id:
                    ...
                    if not log:
                        raise Warning(error)
                    self.create_log_error_dgi('DGI Data Order MPM',self.base_url,'post',error,'PKB')
                    continue
    ...
    @api.multi
    def dgi_mpm_order_wo(self,branch):
        try:
            get_response = self._get_data_wo_mpm_h23(branch)
            if get_response.get('status') == 1:
                ...
                if datas:
                    ...
                    self._process_data_wo_mpm_h23(branch,datas)
            else:
            ...
            _logger.warning("Exception DGI Data Work Order MPM >>>>>>>>> %s"%(err))
            self.create_log_error_dgi('Exception Schedule DGI Data Work Order MPM',self.base_url,'post',err,'PKB')
     

    @api.multi
    def schedule_data_work_order_mpm_h23(self,code):
        ...
        if config_id and branch:
            return config_id.suspend_security().dgi_mpm_order_wo(branch)
        else:
            error = 'Branch Config DGI belum di setting !'
            _logger.warning(error)
            self.create_log_error_dgi('DGI Data Work Order MPM',False,'post',error,'Schedule')            
