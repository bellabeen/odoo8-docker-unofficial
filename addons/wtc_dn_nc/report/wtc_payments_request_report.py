...
class payment_request(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(payment_request, self).__init__(cr, uid, name, context=context)
    ...
    def hari(self):
        data = self.pool.get('wtc.dn.nc').browse(self.cr, self.uid, self.ids)
        data=abs((datetime.strptime(data.date_due,"%Y-%m-%d")- datetime.strptime(data.date,"%Y-%m-%d")).days)
        return data
    
    def pajak(self,data2):
        data2 = self.pool.get('wtc.dn.nc').browse(self.cr, self.uid, self.ids)
...
report_sxw.report_sxw('report.rml.wtc.dn.nc', 'wtc.dn.nc', 'addons/wtc_dn_nc/report/wtc_payments_request_report.rml', parser = payment_request, header = False)