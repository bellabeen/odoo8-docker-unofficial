...
class wtc_kwitansi_dn_nc(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(wtc_kwitansi_dn_nc, self).__init__(cr, uid, name, context=context)
    ...
    def kwitansi(self):
        vals = self.pool.get('wtc.dn.nc').browse(self.cr,self.uid,self.ids)
        ...
        obj_model_id = obj_model.search(self.cr, self.uid,[ ('model','=','wtc.dn.nc') ])[0]
        obj_ir = self.pool.get('ir.actions.report.xml').search(self.cr, self.uid,[('report_name','=','rml.wtc.kwitansi.dn')])
    ...
    def waktu_local(self):
        obj_voucher = self.pool.get('wtc.dn.nc').browse(self.cr, self.uid, self.ids)
        self.pool.get('wtc.register.kwitansi.line').write(self.cr, self.uid,obj_voucher.kwitansi_id.id,{'state': 'printed','dn_nc_id':obj_voucher.id,'amount':obj_voucher.amount})
    ...
    def original(self):
        obj_voucher = self.pool.get('wtc.dn.nc').browse(self.cr, self.uid, self.ids)
        if obj_voucher.type == 'DN':
            origin =  obj_voucher.number
        else :
            origin = obj_voucher.origin or obj_voucher.number
        return origin

report_sxw.report_sxw('report.rml.wtc.kwitansi.dn', 'wtc.dn.nc', 'addons/wtc_account_voucher/report/wtc_kwitansi.rml', parser = wtc_kwitansi_dn_nc, header = False)
