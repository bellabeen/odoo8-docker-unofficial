...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def _get_api_prsl_hso(self, branch, query_date, log=True, noWorkOrder=False):
        ...
        if not self.api_key and not self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H23 Part Sales HSO',api_url,'post',error,'PRSL')
        ...
        response = self.post(name="DGI H23 Part Sales HSO", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
    ...
    @api.multi
    def _get_data_prsl_hso_h23(self, branch, query_date=False, log=True, noWorkOrder=False):
        try:
            #default date is today (for scheduler)
            if not query_date:
                query_date = datetime.now()
            #get wo yang status completed/Approved dan belum di proses sebelumnya
            prsls = self._get_api_prsl_hso(branch, query_date, log, noWorkOrder)
                ...
                for prsl in prsls:
                    ...
                    if noWorkOrder not in daftar_wo:
                        ...
                        if len(parts) == 0:
                            ...
                            if not log:
                                raise Warning(error)
                            self.create_log_error_dgi('DGI Data Part Sales HSO',self.base_url,'post',error,'PRSL')
                            continue
            ...
            _logger.warning("Exception DGI Data Part Sales HSO >>>>>>>>> %s"%(err))
            if not log:
                raise Warning(err)
            self.create_log_error_dgi('Exception DGI Data Part Sales HSO',self.base_url,'post',err,'PRSL')


    @api.multi
    def _process_data_prsl_hso_h23(self, branch, datas, log=True):
    ...
    @api.multi
    def dgi_hso_order_prsl(self,branch):
        try:
            get_response = self._get_data_prsl_hso_h23(branch)
            if get_response.get('status') == 1:
                ...
                if datas:
                    ...
                    self._process_data_prsl_hso_h23(branch,datas)
            else:
            ...
            _logger.warning("Exception DGI Data Part Sales HSO >>>>>>>>> %s"%(err))
            self.create_log_error_dgi('Exception Schedule DGI Data Part Sales HSO',self.base_url,'post',err,'PRSL')
     

    @api.multi
    def schedule_data_work_order_prsl_hso_h23(self,code):
        ...
        if config_id and branch:
            return config_id.suspend_security().dgi_hso_order_prsl(branch)
        else:
            error = 'Branch Config DGI belum di setting !'
            _logger.warning(error)
            self.create_log_error_dgi('DGI Data Part Sales HSO',False,'post',error,'Schedule')            
