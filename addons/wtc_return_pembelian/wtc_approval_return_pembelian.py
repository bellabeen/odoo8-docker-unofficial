...
class wtc_approval_return_pembelian(models.Model):
    _inherit = 'wtc.return.pembelian'
...
class wtc_reason_reject_approval(models.TransientModel):
    _name = "wtc.reason.reject.approval.return.pembelian"
    ...
    def wtc_reject_approval(self, context=None):
        ...
        line = self.env['wtc.return.pembelian'].browse(po_id,context=context)
        objek = False
                    ...
                    if y == x.group_id :
                        ...
                        for z in line.approval_ids :
                            if z.reason == False :
                                ...
                                self.env['wtc.return.pembelian'].write({'state':'confirm','approval_state':'r'})
        if objek == False :
            raise exceptions.ValidationError("User tidak termasuk group approval")
