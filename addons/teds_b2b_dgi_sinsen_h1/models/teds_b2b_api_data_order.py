...
class B2bApiConfig(models.Model):
    ...
    code = fields.Selection(selection_add=[('SINSEN','SINSEN')])
    
    @api.multi
    def _get_api_spk_sinsen(self, branch, query_date, log=True, idProspect=False, idSpk=False):
        ...
        if not self.api_key and not self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H1 SPK Sinsen',api_url,'post',error,'SPK')
        ...
        response = self.post(name="DGI H1 SPK Sinsen", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
        if response.status_code == 200:
            ...
            data = [d for d in data if d.get('statusSPK') in ('approved','close') and d.get('idSpk') not in registered_spk]
    ...
    @api.multi
    def _get_data_oder_sinsen_h1(self, branch, query_date=False, log=True, idProspect=False,idSpk=False):
        ...
        try:
            ...
            spks = self._get_api_spk_sinsen(branch, query_date, log, idProspect, idSpk)
                ...
                for spk in spks:
                    ...
                    if idSpk not in daftar_spk:
                        ...
                        if not idSalesPeople:
                            ...
                            if log:
                                self.create_log_error_dgi('DGI Data Order Sinsen',self.base_url,'post',error,'SPK')
                            continue
                        ...
                        if not noKTPBPKB and not noKtp:
                            ...
                            if not log:
                                raise Warning(error)
                            self.create_log_error_dgi('DGI Data Order Sinsen',self.base_url,'post',error,'PRSP')
                            continue
                        ...
                        prsp_result = self._get_api_prospek_sinsen(branch, tanggal_spk, log=log, idProspect=idProspect)
                        ...
                        if tipePembayaran == 'Kredit':
                            #-------------- Api Leasing --------------#
                            lsng_result = self._get_api_lsng_sinsen(branch, tanggal_spk, idSpk=idSpk)
                            ...
                            #     if not log:
                            #         raise Warning(error)
                            #     self.create_log_error_dgi('DGI Data Order Sinsen',self.base_url,'post',error,'LSNG')
                            #     continue
                            if lsng:
            ...
            _logger.warning("Exception DGI Data Order Sinsen >>>>>>>>> %s"%(err))
            if not log:
                raise Warning(err)
            self.create_log_error_dgi('Exception DGI Data Order Sinsen',self.base_url,'post',err,'SPK')

    @api.multi
    def _process_data_order_sinsen_h1(self, branch, datas, log=True):
        #------------Data Finco----------#
        ...
        for data in datas:                
            ...
            if namaCustomer:
                lead_vals['name_customer'] = namaCustomer
            ...
            if tipePembayaran == 'Cash':
                tipePembayaran = '1'
            elif tipePembayaran == 'Kredit':
                tipePembayaran = '2'

                data_lsng = data.get('lsng')
                if data_lsng:
            ...
            if not lead_obj:
                if branch.is_allow_dgi_prsp:
                    ...
                    lead = self._get_data_prospek_sinsen(data['prsp'])
    ...
    @api.multi
    def dgi_sinsen_order_spk(self,branch):        
        try:
            get_response = self._get_data_oder_sinsen_h1(branch)
            if get_response.get('status') == 1:
                datas = get_response.get('data')
                if datas:
                    # proses data create sale order draft
                    self._process_data_order_sinsen_h1(branch,datas)
            else:
            ...
            _logger.warning("Exception DGI Data Order Sinsen >>>>>>>>> %s"%(err))
            self.create_log_error_dgi('Exception Schedule DGI Data Order Sinsen',self.base_url,'post',err,'SPK')
            
    
    @api.multi
    def schedule_data_order_sinsen_h1(self,code):
        ...
        if config_id and branch:
            ...
            self.create_log_error_dgi('DGI Data Order Sinsen',False,'post',error,'Schedule')            
    
    @api.multi
    def schedule_data_order_sinsen_pos_h1(self,name):
