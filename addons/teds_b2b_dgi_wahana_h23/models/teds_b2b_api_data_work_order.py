...
class B2bApiConfig(models.Model):
    ...
    code = fields.Selection(selection_add=[('WMS H23','WMS H23')])

    @api.multi
    def _get_api_pkb_wahana(self, branch, query_date, log=True, noWorkOrder=False):
        ...
        if not self.api_key and not self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H23 PKB Wahana',api_url,'post',error,'PKB')
        ...
        response = self.post(name="DGI H23 PKB Wahana", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
        if response.status_code == 200:
            ...
            data = [d for d in data if d.get('noWorkOrder') not in registered_wo]
    ...
    @api.multi
    def _get_data_wo_wahana_h23(self, branch, query_date=False, log=True, noWorkOrder=False):
        try:
            #default date is today (for scheduler)
            if not query_date:
                query_date = datetime.now()
            #get wo yang status completed/Approved dan belum di proses sebelumnya
            wos = self._get_api_pkb_wahana(branch, query_date, log, noWorkOrder)
                ...
                for wo in wos:
                    ...
                    if noWorkOrder not in daftar_wo:
                        ...
                        if len(services) == 0 and len(parts) == 0:
                            ...
                            if not log:
                                raise Warning(error)
                            self.create_log_error_dgi('DGI Data PKB Wahana',self.base_url,'post',error,'PKB')
                            continue
            ...
            _logger.warning("Exception DGI Data PKB Wahana >>>>>>>>> %s"%(err))
            if not log:
                raise Warning(err)
            self.create_log_error_dgi('Exception DGI Data PKB Wahana',self.base_url,'post',err,'PKB')

    @api.multi
    def _process_data_wo_wahana_h23(self, branch, datas, log=True):
        ...
        for data in datas:                
            ...
            if not lot_obj:
                ...
                if not product_unit_id:
                    ...
                    if not log:
                        raise Warning(error)
                    self.create_log_error_dgi('DGI Data Order wahana',self.base_url,'post',error,'PKB')
                    continue
            ...
            for part in parts:
                ...
                # Get Value Pekerjaan dari idJob
                jenis_pekerjaan_value = idJobPart.split('-')[-3]
            ...
            for service in services:
                ...
                # Split idJoobService [0,1,2,3] => 0 kode MOTOR , 1 Jenis Pekerjaan, 2 Value Pekerjaan , 3 Auto Generate
                split_idJob = idJobService.split('-')
                jenis_pekerjaan_value = split_idJob[-3]
                kategori_pekerjaan_value = split_idJob[-2]
                if kategori_pekerjaan_value not in ('1','2'):
    ...
    @api.multi
    def dgi_wahana_order_wo(self,branch):
        try:
            get_response = self._get_data_wo_wahana_h23(branch)
            if get_response.get('status') == 1:
                ...
                if datas:
                    ...
                    self._process_data_wo_wahana_h23(branch,datas)
            else:
            ...
            _logger.warning("Exception DGI Data PKB Wahana >>>>>>>>> %s"%(err))
            self.create_log_error_dgi('Exception Schedule DGI Data PKB Wahana',self.base_url,'post',err,'PKB')
     

    @api.multi
    def schedule_data_work_order_wahana_h23(self,code):
        ...
        if config_id and branch:
            return config_id.suspend_security().dgi_wahana_order_wo(branch)
        else:
            error = 'Branch Config DGI belum di setting !'
            _logger.warning(error)
            self.create_log_error_dgi('DGI Data PKB Wahana',False,'post',error,'Schedule')            
