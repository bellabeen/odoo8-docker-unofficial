...
class wtc_k_account_voucher(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(wtc_k_account_voucher, self).__init__(cr, uid, name, context=context)
    ...
    def kwitansi(self):
        vals = self.pool.get('account.voucher').browse(self.cr,self.uid,self.ids)
        ...
        obj_model_id = obj_model.search(self.cr, self.uid,[ ('model','=','account.voucher') ])[0]
        obj_ir = self.pool.get('ir.actions.report.xml').search(self.cr, self.uid,[('report_name','=','rml.other.receivable')])
        ...
        if not obj_jumlah_cetak :
            vals.write({'cetak_ke':1,'kwitansi_id':False,'reason_cancel_kwitansi':False})
#             vals.write({'cetak_ke':1,'reason_cancel_kwitansi':False})
            ...
            vals.write({'cetak_ke':cetakke,'kwitansi_id':False,'reason_cancel_kwitansi':False})
#             vals.write({'cetak_ke':cetakke,'reason_cancel_kwitansi':False})
    ...
    def waktu_local(self):
        obj_voucher = self.pool.get('account.voucher').browse(self.cr, self.uid, self.ids)
...
report_sxw.report_sxw('report.rml.kwitansi', 'account.voucher', 'addons/account_voucher/report/kwitansi.rml', parser = wtc_k_account_voucher, header = False)