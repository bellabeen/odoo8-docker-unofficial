...
class wtc_surat_jalan(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(wtc_surat_jalan, self).__init__(cr, uid, name, context=context)
    ...
    def jumlah_cetakan(self):
        ...
        obj_model_id = obj_model.search(self.cr, self.uid,[ ('model','=','wtc.surat.jalan') ])[0]
        obj_ir = self.pool.get('ir.actions.report.xml').search(self.cr, self.uid,[('report_name','=','rml.wtc.stock.cetak.surat.jalan')])
...
report_sxw.report_sxw('report.rml.wtc.stock.cetak.surat.jalan', 'wtc.surat.jalan', 'addons/wtc_stock/report/wtc_cetak_surat_jalan.rml', parser = wtc_surat_jalan, header = False)