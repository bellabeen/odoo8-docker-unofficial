...
class B2bApiConfig(models.Model):
    ...
    code = fields.Selection(selection_add=[('DAM','DAM')])
    
    @api.multi
    def _get_api_spk_dam(self, branch, query_date, log=True, idProspect=False, idSpk=False):
        ...
        if not self.api_key and not self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H1 SPK DAM',api_url,'post',error,'SPK')
        ...
        # Config Corporate untuk code POS
        if self.corporate_id:
            body['dealerId'] = self.corporate_id
            
        if idProspect:
            body['idProspect'] = idProspect
  
        response = self.post(name="DGI H1 SPK DAM", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
        if response.status_code == 200:
            ...
            query = """
                SELECT spk.md_reference_spk
                FROM dealer_spk spk
                INNER JOIN wtc_branch b ON b.id = spk.branch_id
                WHERE b.code in ('DDA','DDB','DDR','DDD','DDU','DDK','DDC','DDP','DWT','DWN')
            """
            if idSpk:
                query += " AND spk.md_reference_spk = '%(idSpk)s' " %{'idSpk':idSpk}
            else:
                query += " AND spk.md_reference_spk IS NOT NULL "
            self._cr.execute(query)
            ress = self._cr.fetchall()
            registered_spk = [res[0] for res in ress]
            data = [d for d in data if d.get('statusSPK') in ('SALES_ORDER_SUBMIT','SALES_ORDER_UNIT_ALLOC','DELIVERY_TASKSTREAM','DELIVERY_POD') and d.get('idSpk') not in registered_spk]
    ...
    @api.multi
    def _get_data_oder_dam_h1(self, branch, query_date=False, log=True, idProspect=False,idSpk=False):
        ...
        try:
            ...
            spks = self._get_api_spk_dam(branch, query_date, log, idProspect, idSpk)
            if spks.get('status', 0) == 1:
                ...
                for spk in spks:
                    ...
                    idProspect = spk.get('idProspect',False)
                    idSalesPeople = spk.get('idSalesPeople',False)
                    ...
                    if idSpk not in daftar_spk:
                        ...
                        if not idSalesPeople:
                            ...
                            if log:
                                self.create_log_error_dgi('DGI Data Order DAM',self.base_url,'post',error,'SPK')
                            continue
                        ...
                        if not noKTPBPKB and not noKtp:
                            ...
                            if not log:
                                raise Warning(error)
                            self.create_log_error_dgi('DGI Data Order DAM',self.base_url,'post',error,'PRSP')
                            continue
                        ...
                        #### Masih Belum Bisa !!! sementara data prsp di copy dari spk
                            # prsp_result = self._get_api_prospek_dam(branch, tanggal_spk, log=log, idProspect=idProspect)
                            # prsp = prsp_result.get('data',False)
                        ...
                        if tipePembayaran == 'CREDIT':
                            #-------------- Api Leasing --------------#
                            lsng_result = self._get_api_lsng_dam(branch, tanggal_spk, idSpk=idSpk)
                            ...
                            if lsng_result.get('status', 0)==0 or not lsng:
                                ...
                                if not log:
                                    raise Warning(error)
                                self.create_log_error_dgi('DGI Data Order DAM',self.base_url,'post',error,'LSNG')
                                continue
            ...
            _logger.warning("Exception DGI Data Order DAM >>>>>>>>> %s"%(err))
            if not log:
                raise Warning(err)
            self.create_log_error_dgi('Exception DGI Data Order DAM',self.base_url,'post',err,'SPK')

    @api.multi
    def _process_data_order_dam_h1(self, branch, datas, log=True):
        #------------Data Finco----------#
        ...
        for data in datas:                
            ...
            if tipePembayaran == 'CASH':
                ...
                data_lsng = data.get('lsng',False)
                if data_lsng:
                    idFinanceCompany = data_lsng.get('idFinanceCompany',False)
                    if idFinanceCompany:
                        finco_id = data_fincoy.get(data_lsng.get('idFinanceCompany'))
                        if not finco_id:
                            note_log.append('Data Finance Company %s not found !'%idFinanceCompany)
                    cicilan = data_lsng.get('jumlahCicilan',False)
                    tenor = data_lsng.get('tenor',False)
                    dp = data_lsng.get('jumlahDP',False)
            ...
            # --------------Employee Sales-----------------#
            # --------------Data untuk Multi Cabang, 1 kode MD 3 cabang di TDM ---------------#
            branch_objs = self.env['wtc.branch'].search([('code','in',('DDA','DDB','DDR','DDD','DDU','DDK','DDC','DDP','DWT','DWN'))])
            branch_ids = [b.id for b in branch_objs]
            get_employee = self._get_employee_multi(branch_ids,idSalesPeople)
            if not get_employee:
            ...
            branch_id = get_employee.branch_id.id
            ...
            if not lead_obj:
                if branch.is_allow_dgi_prsp:
                    ...
                    lead = self._get_data_prospek_dam(data['prsp'])
    ...
    @api.multi
    def dgi_dam_order_spk(self,branch):        
        try:
            get_response = self._get_data_oder_dam_h1(branch)
            if get_response.get('status') == 1:
                datas = get_response.get('data')
                if datas:
                    # proses data create sale order draft
                    self._process_data_order_dam_h1(branch,datas)
            else:
            ...
            _logger.warning("Exception DGI Data Order DAM >>>>>>>>> %s"%(err))
            self.create_log_error_dgi('Exception Schedule DGI Data Order DAM',self.base_url,'post',err,'SPK')
            
    
    @api.multi
    def schedule_data_order_dam_h1(self,code):
        ...
        if config_id and branch:
            return config_id.suspend_security().dgi_dam_order_spk(branch)
        else:
            error = 'Branch Config DGI belum di setting !'
            _logger.warning(error)
            self.create_log_error_dgi('DGI Data Order DAM',False,'post',error,'Schedule')            

    @api.multi
    def schedule_data_order_dam_pos_h1(self,name):
        config_id = self.env['teds.b2b.api.config'].suspend_security().search([
            ('code','=','DAM'),
            ('name','=',name)],limit=1)
        branch = config_id.branch_id
        if config_id and branch:
            return config_id.suspend_security().dgi_sinsen_order_spk(branch)
        else:
            error = 'Branch Config DGI %s belum di setting !'%name
            _logger.warning(error)
            self.create_log_error_dgi('DGI Data Order DAM',False,'post',error,'Schedule')            
