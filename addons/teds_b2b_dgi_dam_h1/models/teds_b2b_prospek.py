...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def _get_api_prospek_dam(self, branch, query_date, log=True, idProspect=False):
        api_url = "%s/prsp/read" % self.base_url
        date_format = '%Y-%m-%d %H:%M:%S'

        from_time = datetime.combine(query_date - timedelta(days=6), datetime.min.time())
        ...
        response = self.post(name="DGI H1 PRSP DAM", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
        if response.status_code == 200:
            ...
            if not data:
                ...
                if log:
                    self.create_log_error_dgi('DGI H1 SPK DAM',api_url,'post',error,'PRSP')
                return {'status':0,'error':error}
    ...
    @api.multi
    def _get_data_prospek_dam(self, vals):
        ...
        if idSalesPeople:
            get_employee = self.env['teds.b2b.api.config']._get_employee_multi(
                branch_ids, idSalesPeople)
            if get_employee:
        ...
        if not noKontak.isdigit() or len(noKontak) < 5 :
            noKontak = False
        ...
        # Metode Follow Up #
            ...
            # konversi -> ke poin dibawah dan ditambahkan SMS
            # 1 = SMS
            # 2 = Telepon -> 151=Telepon
            # 3 = Visit -> 152=Kunjungan
            # 4 = Direct Touch -> 154=Dealer, 155=Event, 156=Pos, 153=Walkin

        metodeFollowUp = prsp.get('metodeFollowUp')
        if metodeFollowUp == '1':
            metodeFollowUp = 'MESSAGE'
        elif metodeFollowUp == '2':
            metodeFollowUp = 'CALL'
        elif metodeFollowUp == '3':
