...
import logging
_log = logging.getLogger('neraca_report')
...
class report_account_common(report_sxw.rml_parse, common_report_header):
    ...
    def get_laba_rugi(self, new_context):
        ...
        total_pendapatan=total_pendapatan!=0 and (total_pendapatan*-1) or total_pendapatan
        total_biaya=0
    ...
    def get_lines(self, data):
        ...
        #bagian aktiva
        report_h = [
            {'code': 'neraca_harta_lcr','name':'Harta Lancar'},
            {'code': 'neraca_harta_ttp','name':'Harta Tetap'},
            {'code': 'neraca_harta_lain','name':'Harta Lainnya'},
            ]
        ...
        for report in report_h:
            ...
            if isyear:
                ...
                while m<=12:
                    ...
                    m+=1
            lines.append(vals_view)
            total_harta+=total_head        
            total_harta_cmp+=total_head_cmp        
            if isyear:
                m=2
                while m<=12:
                    total_harta_m[m]+=total_head_m[m]
                    m+=1
                       
        vals_view = {
            'name': 'Total Harta',
            'balance': total_harta,
            'balance_cmp': total_harta_cmp,
            'type': 'view',
            'level': 0,
            'account_type': 'view', 
        }
        if isyear:
            m=2
            while m<=12:
                vals_view['balance_cmp_m'+str(m)] = total_harta_m[m]
                m+=1
        ...
        #passiva
        report_k = [
            {'code': 'neraca_kewajiban_lcr','name':'Hutang Lancar'},
            {'code': 'neraca_kewajiban_pjg','name':'Hutang Jangka Panjang'},
            {'code': 'neraca_kewajiban_lain','name':'Hutang Lainnya'},
            {'code': 'neraca_modal','name':'Modal dan Laba Ditahan'},
            ]
        vals_view = {
        ...
        for report in report_k:
            ...
            if account_ids:
                for account in account_obj.browse(self.cr, self.uid, account_ids, context=used_context):                        
                    if account.smcus_laba_rugi:
                        balance = self.get_laba_rugi(used_context)
                    else:
                        balance = account.balance
                        if balance!=0:
                    ...
                    if iscmp:
                        balance_cmp=0
                        if account.smcus_laba_rugi:
                            balance_cmp = self.get_laba_rugi(used_context_2)
                        else:
                            balance_cmp = account_obj.browse(self.cr, self.uid, account.id, context=used_context_2).balance
                            if balance_cmp!=0:
                                balance_cmp = balance_cmp*-1
                    ...
                    if isyear:
                        ...
                        while m<=12:
                            if account.smcus_laba_rugi:
                                balance_m = self.get_laba_rugi(nextcontext[m])
                            else:
                                balance_m = account_obj.browse(self.cr, self.uid, account.id, context=nextcontext[m]).balance
                                if balance_m!=0:
                                    balance_m = balance_m*-1
            ...
            if isyear:
                ...
                while m<=12:
                    ...
                    m+=1
            lines.append(vals_view)
            total_hutang+=total_head        
            total_hutang_cmp+=total_head_cmp        
            if isyear:
                m=2
                while m<=12:
                    total_hutang_m[m]+=total_head_m[m]
                    m+=1
                       
        vals_view = {
            'name': 'Total Kewajiban dan Modal',
            'balance': total_hutang,
            'balance_cmp': total_hutang_cmp,
            'type': 'view',
            'level': 0,
            'account_type': 'view', 
        }
        if isyear:
            m=2
            while m<=12:
                vals_view['balance_cmp_m'+str(m)] = total_hutang_m[m]
                m+=1
...
report_sxw.report_sxw('report.smcus.neraca.report.zhr', 'account.financial.report',
    'addons/wtc_account_report_zhr/report/neraca_report.rml', parser=report_account_common, header='internal')
report_sxw.report_sxw('report.smcus.neraca.cmp.report.zhr', 'account.financial.report',
    'addons/wtc_account_report_zhr/report/neraca_report_cmp.rml', parser=report_account_common, header='internal')
report_sxw.report_sxw('report.smcus.neraca.year.report.zhr', 'account.financial.report',
    'addons/wtc_account_report_zhr/report/neraca_report_year.rml', parser=report_account_common, header='internal landscape')
