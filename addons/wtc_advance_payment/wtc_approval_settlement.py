...
class wtc_reason_reject_approval(models.TransientModel):
    _name = "wtc.reason.reject.approval.stl"
    ...
    def wtc_reject_approval(self, context=None):
        ...
        line = self.env['wtc.settlement'].browse(po_id,context=context)
        objek = False
                    ...
                    if y == x.group_id :
                        ...
                        for z in line.approval_ids :
                            if z.reason == False :
                                ...
                                self.env['wtc.settlement'].write( {'state':'draft','approval_state':'r'})
        if objek == False :
            raise Warning("User tidak termasuk group approval")
