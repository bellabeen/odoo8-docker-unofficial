...
class dealer_sale_order(report_sxw.rml_parse):
    ...
    def invoice_id(self):
        ...
        invoice2 = self.pool.get('account.invoice').search(self.cr, self.uid,[ ('origin','=',invoice),('tipe','=','finco') ])
        no_invoice = self.pool.get('account.invoice').browse(self.cr, self.uid,invoice2).number
        return no_invoice
    
report_sxw.report_sxw('report.rml.dealer.sale.order.pelunasan', 'dealer.sale.order', 'addons/dealer_sale_order/report/dealer_sale_order_pelunasan_leasing_report.rml', parser = dealer_sale_order, header = False)