...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def _get_api_lsng_cdn(self, branch, query_date, log=True, idProspect=False, idSpk=False):
        ...
        if not self.api_key and self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H1 SPK CDN',api_url,'post',error,'LSNG')
        ...
        response = self.post(name="DGI H1 LSNG CDN",url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
