...
class wtc_reject_approval_stock_mutation(models.TransientModel):
    _name = "wtc.reject.approval.stock.distribution"
    ...
    def wtc_reject_approval(self, context=None):
        ...
        line = self.env['wtc.stock.distribution'].browse(po_id,context=context)
        objek = False
                    ...
                    if y == x.group_id :
                        ...
                        for z in line.approval_ids :
                            if z.reason == False :
                                ...
                                self.env['wtc.stock.distribution'].write({'state':'confirm','approval_state':'r'})
        if objek == False :
            raise exceptions.ValidationError("User tidak termasuk group approval")
