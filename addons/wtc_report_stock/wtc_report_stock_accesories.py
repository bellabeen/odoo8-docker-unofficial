...
class wtc_report_stock_accesories(osv.osv_memory):
    _name = "wtc.report.stock.accesories.wizard"
    _description = "Report Stock Accesories"
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(wtc_report_stock_accesories, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    _columns = {
        ...
        'options': fields.selection([('stock','Stock')], 'Options', required=True, change_default=True, select=True),
        'branch_ids': fields.many2many('wtc.branch', 'wtc_report_stock_accesories_branch_rel', 'wtc_report_stock_accesories_wizard_id',
                                        'branch_id', 'Branch', copy=False),
        'product_ids': fields.many2many('product.product', 'wtc_report_stock_accesories_product_rel', 'wtc_report_stock_sparepart_wizard_id',
                                        'product_id', 'Product', copy=False, ),
        'location_ids': fields.many2many('stock.location', 'wtc_report_stock_accesories_location_rel', 'wtc_report_stock_sparepart_wizard_id',
    ...
    def _print_excel_report(self, cr, uid, ids, data, context=None):        
        ...
        query_group="group by l.branch_id,p.id,q.id, q.in_date, consolidated_date, q.reservation_id, m.id, move_origin, picking_name ,product_name, product_desc, categ_name, location_name, branch_code, branch_name,profit_centre, ppb.cost "      
        ...
        query = """
            ...
            FROM 
                ...
                WHERE 1=1 AND (c.name = 'Extras' or c2.name = 'Extras')
        ...
        wbf=self.wbf
        worksheet = workbook.add_worksheet('Stock Accesories')
        ...
        filename = 'Report Stock Accesories '+str(date)+'.xlsx'        
        worksheet.write('A1', company_name , wbf['company'])
        worksheet.write('A2', 'Report Stock Accesories' , wbf['title_doc'])
        ...
        for res in ress:
            ...
            location_name = str(res[6].encode('ascii','ignore').decode('ascii')) if res[6] != None else ''
            aging =res[7]
            qty_titipan = res[8]
            qty_reserved = res[9]
            qty_stock = res[10]
        ...
        form_res = ir_model_data.get_object_reference(cr, uid, 'wtc_report_stock', 'view_report_stock_accesories_wizard')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'wtc.report.stock.accesories.wizard',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
