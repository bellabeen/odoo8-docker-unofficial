...
class InsentiveSalesReport(models.TransientModel):
    ...
    def sales_partner_report(self):
        ...
        for res in ress:
            ...
            persen_credit = float(total_credit) / total_unit
            ...
            if results.get(res.get('branch_code')):
                ...
                results[res['branch_code']]['rupiah_reward'] += rupiah_reward
            ...
            result_details.append({
                ...
                'rupiah_persen_absensi':rupiah_absensi,
        ...
        worksheet_summary.set_column('L1:L1', 20)
        ...
        worksheet_detail.set_column('K1:K1', 23)
        worksheet_detail.set_column('L1:L1', 23)
        worksheet_detail.set_column('M1:M1', 23)
        worksheet_detail.set_column('N1:N1', 23)
        worksheet_detail.set_column('O1:O1', 23)
        worksheet_detail.set_column('P1:P1', 20)
        ...
        worksheet_detail.write('N5', 'Rupiah Persen Mediator' , wbf['header'])
        worksheet_detail.write('O5', 'Rupiah Persen Absensi' , wbf['header'])
        worksheet_detail.write('P5', 'Total Insentif' , wbf['header'])
        ...
        sum_rupiah_persen_mediator = 0
        sum_rupiah_persen_absensi = 0
        ...
        for detail in result_details:
            ...
            worksheet_detail.write('N%s' % row_dt, detail['rupiah_persen_mediator'] , wbf['content_float'])
            worksheet_detail.write('O%s' % row_dt, detail['rupiah_persen_absensi'] , wbf['content_float'])
            worksheet_detail.write('P%s' % row_dt, detail['total_insentif'] , wbf['content_float'])
            ...
            sum_rupiah_persen_mediator += detail['rupiah_persen_mediator']
            sum_rupiah_persen_absensi += detail['rupiah_persen_absensi']
        ...
        formula_total_reward_sm = '{=subtotal(9,H%s:H%s)}' % (row_sm1, row_sm-1)
        ...
        formula_rupiah_persen_mediator = '{=subtotal(9,N%s:N%s)}' % (row_dt1, row_dt-1)
        formula_rupiah_persen_absensi = '{=subtotal(9,O%s:O%s)}' % (row_dt1, row_dt-1)
        formula_total_insentif = '{=subtotal(9,P%s:P%s)}' % (row_dt1, row_dt-1)
        ...
        worksheet_summary.write_formula(row_sm-1,7,formula_total_reward_sm, wbf['total_number_float'], sum_reward)
        ...
        worksheet_detail.write_formula(row_dt-1,13,formula_rupiah_persen_mediator, wbf['total_number_float'], sum_rupiah_persen_mediator)
        worksheet_detail.write_formula(row_dt-1,14,formula_rupiah_persen_absensi, wbf['total_number_float'], sum_rupiah_persen_absensi)
        
        worksheet_detail.write_formula(row_dt-1,15,formula_total_insentif, wbf['total_number_float'], sum_total_insentif)
