...
class ImportAbsensi(osv.osv_memory):
    _name = "wtc.import.absensi"
    _columns = {
                'name': fields.char('File Name', 16),
                'data_file': fields.binary('File'),
                }   
    def import_excel(self, cr, uid, ids, context=None):
        obj_absensi = self.pool.get('wtc.absensi') 
        ...
        for rx in range(1,sh.nrows):  
            ...
            if not check_obj_absensi :       
                ...
                obj_absensi.write(cr,uid,absensi_id.id,{'total_absensi': total_absensi,'jumlah_hari_kerja':jumlah_hari_kerja,'sp':sp}, context=context)
        return True