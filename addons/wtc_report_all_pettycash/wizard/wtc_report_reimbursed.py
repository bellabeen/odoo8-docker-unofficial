...
class wtc_report_reimbursed(osv.osv_memory):
    ...
    def _print_excel_report_reimbursed(self, cr, uid, ids, data, context=None):  
        branch_ids = data['branch_ids']
        division = data['division']
        journal_id = data['journal_id']
        state_reimburse = data['state_reimburse']
        ...
        query = """
                ...
                r.division as r_division, 
                j.name as journal_name, 
                r.date_approve as date_approve, 
                r.date_request as date_request, 
                r.date_cancel as date_cancel,
                l.name as line_name, 
                l.date as line_date, 
                bl.code as branch_destination, 
                l.amount as line_amount,
                r.state as r_state
                from wtc_reimbursed r 
                left join wtc_pettycash l ON l.reimbursed_id = r.id
                left join account_journal j ON j.id = r.journal_id
                left join wtc_branch b ON b.id = r.branch_id
                left join wtc_branch bl ON bl.id = l.branch_id 
        ...
        if start_date :
            query_end += " AND r.date_request >= '%s' " % str(start_date)
        if end_date :
            query_end += " AND r.date_request <= '%s' " % str(end_date)
        if division :
            query_end += " AND r.division = '%s'" % str(division) 
        if state_reimburse :
            query_end += " AND r.state = '%s'" % str(state_reimburse) 
        if journal_id :
            query_end += " AND r.journal_id = '%s'" % str(journal_id)             
            
        query_order = "order by b.code,r.date_request"
        ...
        wbf=self.wbf
        worksheet1 = workbook.add_worksheet('Report Reimburse Petty Cash')
        worksheet2 = workbook.add_worksheet('Report Reimburse Details')
        ...
        filename = 'Report Reimburse Petty Cash'+str(date)+'.xlsx'        
        worksheet1.write('A1', company_name , wbf['company'])
        worksheet1.write('A2', 'Report Reimburse' , wbf['title_doc'])
        ...
        worksheet2.write('A2', 'Report Reimburse Details' , wbf['title_doc'])
        ...
        worksheet2.write('B%s' % (row+1), 'Reimburse Ref' , wbf['header'])
        worksheet2.write('C%s' % (row+1), 'Branch' , wbf['header'])
        worksheet2.write('D%s' % (row+1), 'Division' , wbf['header'])
        worksheet2.write('E%s' % (row+1), 'Date Request' , wbf['header'])
        worksheet2.write('F%s' % (row+1), 'Date Approve' , wbf['header'])
        worksheet2.write('G%s' % (row+1), 'Date Cancel' , wbf['header'])
        worksheet2.write('H%s' % (row+1), 'Description' , wbf['header'])
        worksheet2.write('I%s' % (row+1), 'Date' , wbf['header'])
        worksheet2.write('J%s' % (row+1), 'Branch Destination' , wbf['header'])
        ...
        for res in all_lines:
            r_id = res[0]
            r_name = res[1].encode('ascii','ignore').decode('ascii') if res[1] != None else ''
            branch_code = res[2].encode('ascii','ignore').decode('ascii') if res[2] != None else ''
            r_division =  res[3].encode('ascii','ignore').decode('ascii') if res[3] != None else ''
            journal_name = res[4].encode('ascii','ignore').decode('ascii') if res[4] != None else ''
            date_approve = res[5]
            date_request = res[6]
            date_cancel =  res[7] if res[7] != None else 0
            line_name = res[8]
            line_date = res[9]
            branch_destination = res[10]
            line_amount = res[11]
            r_state = res[12]
            ...
            worksheet1.write('B%s' % row, r_name, wbf['content_number'])
            ...
            worksheet2.write('B%s' % row, r_name, wbf['content'])  
            worksheet2.write('C%s' % row, branch_code, wbf['content'])
            worksheet2.write('D%s' % row, r_division, wbf['content'])
            worksheet2.write('E%s' % row, date_request, wbf['content_date'])
            worksheet2.write('F%s' % row, date_approve, wbf['content_date'])
            worksheet2.write('G%s' % row, date_cancel, wbf['content_date'])
            worksheet2.write('H%s' % row, line_name, wbf['content'])
            worksheet2.write('I%s' % row, line_date, wbf['content'])   
            worksheet2.write('J%s' % row, branch_destination, wbf['content']) 
            worksheet2.write('K%s' % row, r_state, wbf['content'])      
            worksheet2.write('L%s' % row, line_amount, wbf['content_float'])
        ...
        worksheet2.autofilter('A7:K%s' % (row))  
        ...
        worksheet2.merge_range('D%s:K%s' % (row,row), '', wbf['total'])
        ...
        ##sheet 2
        formula_total_line_amount = '{=subtotal(9,L%s:L%s)}' % (row1, row-1) 
        ...
        ##sheet 2
        worksheet2.write_formula(row-1,11,formula_total_line_amount, wbf['total_float'], line_amount) 
