...
class wtc_report_incentive_sales_counter(osv.osv_memory):
    _inherit = "wtc.report.incentive.sale.wizard"
    
    wbf = {}
    
    def _print_excel_report_sales_counter(self, cr, uid, ids, data, context=None): 
        ...
        wbf=self.wbf
        worksheet = workbook.add_worksheet('Incentive Sales Counter')
        ...
        filename = 'Report Incentive Sales Counter '+str(date)+'.xlsx'        
        worksheet.write('A1', company_name , wbf['company'])
        worksheet.write('A2', 'Report Incentive Sales Counter' , wbf['title_doc'])
...
#         worksheet.merge_range('AF%s:AF%s' % (row+1,row+2), 'Punishment' , wbf['header'])
        ...
        for res in ress:
            ...
            [incentive_cash, incentive_credit, incentive_reward]=self._get_incentive(cr,uid,ids,sales_type,cluster,total_unit,total_unit,0,nip_sales,context) 
            ...
            total_incentive=(incentive_cash_amount+incentive_credit_amount)-(incentive_ditahan+potongan_mediator_all)
            ...
            worksheet.write('J%s' % row, ' ' , wbf['content_float'])
            ...
            worksheet.write('Z%s' % row, '0' , wbf['content_float'])
...
#         worksheet.freeze_panes(6, 5)
        ...
        worksheet.write('Z%s' % (row), '', wbf['total'])
        ...
        formula_total_incentive = '{=subtotal(9,AC%s:AC%s)}' % (row1, row-1) 
        formula_total_incentive_fix = '{=subtotal(9,AD%s:AD%s)}' % (row1, row-1)  
