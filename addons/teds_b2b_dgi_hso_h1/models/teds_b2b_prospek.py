...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def _get_api_prospek_hso(self, branch, query_date, log=True, idProspect=False):
        ...
        response = self.post(name="DGI H1 PRSP HSO", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
        if response.status_code == 200:
            ...
            if not data:
                ...
                if log:
                    self.create_log_error_dgi('DGI H1 SPK HSO',api_url,'post',error,'PRSP')
                return {'status':0,'error':error}
    ...
    @api.multi
    def _get_data_prospek_hso(self, vals):
        ...
        if not noKontak.isdigit() or len(noKontak) < 5 :
            noKontak = noKontak.replace('+62','08').replace('-','')
