...
class B2bApiConfig(models.Model):
    ...
    @api.multi
    def _get_api_spk_hso(self, branch, query_date, log=True, idProspect=False, idSpk=False):
        ...
        if not self.api_key and not self.api_secret:
            ...
            if log:
                self.create_log_error_dgi('DGI H1 SPK HSO',api_url,'post',error,'SPK')
        ...
        response = self.post(name="DGI H1 SPK HSO", url=api_url, body=body, headers=headers, type='incoming', verify=self.verify)
        if response.status_code == 200:
            ...
            data = [d for d in data if d.get('statusSPK') in ('ProcessLeveling','Complete') and d.get('idSpk') not in registered_spk]
    ...
    @api.multi
    def _get_data_oder_hso_h1(self, branch, query_date=False, log=True, idProspect=False,idSpk=False):
        ...
        try:
            ...
            spks = self._get_api_spk_hso(branch, query_date, log, idProspect, idSpk)
                ...
                for spk in spks:
                    ...
                    if idSpk not in daftar_spk:
                        ...
                        if not idSalesPeople:
                            ...
                            if log:
                                self.create_log_error_dgi('DGI Data Order HSO',self.base_url,'post',error,'SPK')
                            continue
                        ...
                        if not noKtp:
                            ...
                            if not log:
                                raise Warning(error)
                            self.create_log_error_dgi('DGI Data Order HSO',self.base_url,'post',error,'PRSP')
                            continue
                        ...
                        if not noKTPBPKB:
                            spk['noKTPBPKB'] = noKtp
                            noKTPBPKB = noKtp
                        if not spk.get('namaBPKB'):
                            spk['namaBPKB'] = spk.get('namaCustomer','')
                        ...
                        prsp_result = self._get_api_prospek_hso(branch, tanggal_spk, log=log, idProspect=idProspect)
                        ...
                        if tipePembayaran == 'Credit':
                            #-------------- Api Leasing --------------#
                            lsng_result = self._get_api_lsng_hso(branch, tanggal_spk, idSpk=idSpk)
                            ...
                            if lsng_result.get('status', 0)==0 or not lsng:
                                ...
                                if not log:
                                    raise Warning(error)
                                self.create_log_error_dgi('DGI Data Order HSO',self.base_url,'post',error,'LSNG')
                                continue
            ...
            _logger.warning("Exception DGI Data Order HSO >>>>>>>>> %s"%(err))
            if not log:
                raise Warning(err)
            self.create_log_error_dgi('Exception DGI Data Order HSO',self.base_url,'post',err,'SPK')

    @api.multi
    def _process_data_order_hso_h1(self, branch, datas, log=True):
        #------------Data Finco----------#
        ...
        for data in datas:                
            ...
            if not lead_obj:
                if branch.is_allow_dgi_prsp:
                    ...
                    lead = self._get_data_prospek_hso(data['prsp'])
    ...
    @api.multi
    def dgi_hso_order_spk(self,branch):        
        try:
            get_response = self._get_data_oder_hso_h1(branch)
            if get_response.get('status') == 1:
                datas = get_response.get('data')
                if datas:
                    # proses data create sale order draft
                    self._process_data_order_hso_h1(branch,datas)
            else:
            ...
            _logger.warning("Exception DGI Data Order HSO >>>>>>>>> %s"%(err))
            self.create_log_error_dgi('Exception Schedule DGI Data Order HSO',self.base_url,'post',err,'SPK')
            
    
    @api.multi
    def schedule_data_order_hso_h1(self,code):
        ...
        if config_id and branch:
            return config_id.suspend_security().dgi_hso_order_spk(branch)
        else:
            error = 'Branch Config DGI belum di setting !'
            _logger.warning(error)
            self.create_log_error_dgi('DGI Data Order HSO',False,'post',error,'Schedule')            